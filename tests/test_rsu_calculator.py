...
class TestRSUCalculator(unittest.TestCase):
    def test_match_transactions_full_sell(self):
        tx1 = Transaction(
            date=datetime(2023, 1, 15), action=TransactionAction.VEST, quantity=50, symbol="AAPL"
        )
        tx2 = Transaction(
            date=datetime(2023, 6, 15), action=TransactionAction.SELL, quantity=50, symbol="AAPL"
        )
    ...
    def test_match_transactions_partial_sell_and_unsold(self):
        ...
        tx2 = Transaction(
            date=datetime(2023, 3, 10), action=TransactionAction.VEST, quantity=20, symbol="GOOG"
        )
        tx3 = Transaction(
            date=datetime(2023, 6, 15), action=TransactionAction.SELL, quantity=20, symbol="AAPL"
        )
    ...
    def test_get_historical_price(self, mock_ticker):
        mock_hist_df = pd.DataFrame({"Close": [150.0]}, index=pd.to_datetime(["2023-01-13"]))
    ...
    def test_calculate_rsu_metrics_with_sell(self):
        ...
        data = [
            {"Ticker": "AAPL", "Vest Date": "2023-01-15", "Sell Date": "2023-06-15", "Shares": 50}
        ]

        results = calculate_rsu_metrics(
            data, price_fetcher=mock_price_fetcher, rate_fetcher=mock_rate_fetcher
        )
    ...
    def test_calculate_rsu_metrics_no_sell(self):
        ...
        data = [{"Ticker": "GOOG", "Vest Date": "2023-03-10", "Sell Date": None, "Shares": 20}]
